    ...
    if (empty($login_or_email) || empty($haslo)) {
        ...
        if ($result->num_rows == 1) {
            ...
            if (password_verify($haslo, $row["haslo"])) {
                ...
                header("Location: http://localhost/studia/SMARTHOME/mobile/konto.php");
            } else {
                $error = "Błędne hasło.";
            }
        } else {
            $error = "Błędny login lub email.";
            header("Location: http://localhost/studia/SMARTHOME/mobile/login.php?error=" . urlencode($error));
        }
    }
}
