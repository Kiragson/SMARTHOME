...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($resultCheck->num_rows > 0) {
        ...
        header('Location: house.php?error=' . urlencode($errorMessage));
        exit;
    }
    ...
    if ($stmt->execute()) {
        ...
        header('Location: house.php?message=' . urlencode($response['message']));
    } else {
        ...
        header('Location: new_device.php?message=' . urlencode($response['message']));
    }
